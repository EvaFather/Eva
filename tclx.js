 ...
 * 同程旅行签到 - 多账号版
 * 
 * 抓包说明: 在 APP 打开「领福利」页触发 signIndex 即可自动抓取 Header。
 * 多账号配置: 支持在本地数据中用 '#' 或 '@' 或 换行 分隔多个账号 Header 的 JSON 字符串。
 */
...
function handleGetCookie() {
  ...
  const phone = reqHeaders.phone || reqHeaders.Phone || '未知账号'
  const newHeaderStr = JSON.stringify(reqHeaders)
  
  let rawVal = $.getdata(KEY_SIGNHEADER) || ''
  let accountList = []

  if (rawVal) {
    // 解析已存账号
    accountList = rawVal.split(/[\n#@]/).filter(item => item.trim() !== '')
  }

  // 判断是否已有该账号，已有则更新，无则新增
  let updated = false
  for (let i = 0; i < accountList.length; i++) {
    try {
      const existHeader = JSON.parse(accountList[i])
      const existPhone = existHeader.phone || existHeader.Phone
      if (existPhone && existPhone === phone) {
        accountList[i] = newHeaderStr
        updated = true
        break
      }
    } catch (e) {}
  ...
  if (!updated) {
    accountList.push(newHeaderStr)
  }

  const saveVal = accountList.join('#')
  $.setdata(saveVal, KEY_SIGNHEADER)
  
  const title = `🎉 获取同程账号成功 [${phone}]`
  const subTitle = updated ? '已更新现有账号 Cookie' : `已新增账号，当前共 ${accountList.length} 个账号`
  $.log(`${title} - ${subTitle}`)
  $.msg($.name, title, subTitle)
}

// 单账号任务执行
async function runAccount(headers, index, total) {
  const phone = headers.phone || headers.Phone || `账号[${index + 1}]`
  $.log(`\n================== 开始执行 账号 [${index + 1}/${total}]: ${phone} ==================`)
  
  let accountLog = `📱 账号 [${index + 1}]: ${phone}\n`
  let signSuccess = false

  const signIndexRes = await signIndex(headers)
  if (!signIndexRes || signIndexRes.code !== 2200) {
    $.log(`用户【${phone}】 - Token 失效，请重新抓包`)
    accountLog += `❌ Token 已失效，请重新登录抓包\n\n`
    return accountLog
  }
  ...
  if (todaySign) {
    ...
    $.log(`用户【${phone}】 - 开始执行每日签到`)
    const signRes = await doSignIn(headers)
    if (signRes && signRes.code === 2200) {
      $.log(`用户【${phone}】 - 签到成功！`)
      signSuccess = true
    } else {
      $.log(`用户【${phone}】 - 签到失败：${signRes?.message || '未知原因'}`)
    }
  }

  // 任务流程
  const taskListRes = await getTaskList(headers)
  if (taskListRes && taskListRes.code === 2200 && Array.isArray(taskListRes.data)) {
    ...
    for (const task of tasks) {
      ...
      $.log(`用户【${phone}】 - 做任务【${title}】，需等待 ${browserTime} 秒`)
      const startRes = await taskStart(taskCode, headers)
      if (startRes && startRes.code === 2200 && startRes.data) {
        const taskId = startRes.data
        await $.wait(browserTime * 1000)
        let finishOk = false
        for (let attempt = 0; attempt < 3; attempt++) {
          const finishRes = await taskFinish(taskId, headers)
        ...
        if (finishOk) {
          await taskReceive(taskId, headers)
          $.log(`用户【${phone}】 - 任务【${title}】完成并成功领取奖励`)
        }
      }
    }
  }
  ...
  // 获取刷新后的里程
  const finalIndexRes = await postApi('/index/signIndex', {}, headers)
  if (finalIndexRes && finalIndexRes.code === 2200 && finalIndexRes.data) {
    const d = finalIndexRes.data
    const cycleSignNum = d.cycleSighNum
    const finalMileage = d.mileageBalance?.mileage ?? 0
    const todayMileage = d.mileageBalance?.todayMileage ?? 0
    
    const statusText = signSuccess ? '✨ 签到成功' : '❗️ 签到失败/手动'
    accountLog += `${statusText} | 本月签到 ${cycleSignNum} 天\n🎁 当前里程: ${finalMileage} (+${todayMileage})\n\n`
  } else {
    accountLog += signSuccess ? '✅ 签到成功 (获取最终里程失败)\n\n' : '❌ 签到失败\n\n'
  }
...
// 主入口
async function main() {
  const rawData = $.getdata(KEY_SIGNHEADER)
  if (!rawData) {
    $.log('未检测到账号 Cookie，请先开启重写并在同程旅行 APP「领福利」页面触发签到！')
    $.msg($.name, '⚠️ 未获取到 Cookie', '请先在 APP 内打开领福利页面获取账号')
    return
  }

  // 兼容多账号分隔符 (#、@、换行)
  const accountStrings = rawData.split(/[\n#@]/).filter((item) => item.trim() !== '')
  $.log(`共检测到 ${accountStrings.length} 个账号，开始按顺序执行签到...`)

  let summaryNotice = ''

  for (let i = 0; i < accountStrings.length; i++) {
    try {
      const headers = JSON.parse(accountStrings[i].trim())
      const resultLog = await runAccount(headers, i, accountStrings.length)
      summaryNotice += resultLog
    } catch (e) {
      $.log(`第 ${i + 1} 个账号数据解析失败，跳过该账号`)
    }
    if (i < accountStrings.length - 1) {
      await $.wait(3000) // 账号间随机/间隔延迟
    }
  }

  $.msg(`✈️ ${$.name} 运行完成`, `共执行 ${accountStrings.length} 个账号`, summaryNotice.trim())
}
