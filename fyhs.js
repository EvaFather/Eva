...
#  ① 请求-header 拦任何业务接口请求头 → 自动抓 Authorization(token), 老端打开即触发, 无需重登 ← 推荐
#  ② response-body  拦 /auth/wx/login 响应 → 拿 token+userId+手机号(能做同会员去重), 但需重登/清缓存才触发
# (以下为各端配置示例, 脚本参数/语法按各端格式; pattern 以下用请求头模式①)
# Loon:
#   [Script]
#   http-请求 ^https:\/\/openapp\.fmy90\.com\/(user|shop|sign|active|step|home)\/ script-path=https://gist.githubusercontent.com/Sliverkiss/d744c74d780f47cd85f6980ca5014170/raw/fyhs.js, tag=飞蚂蚁回收抓包
#   [MITM] hostname = openapp.fmy90.com
# Surge:
#   [Script] type=http-请求, pattern=^https:\/\/openapp\.fmy90\.com\/(user|shop|sign|active|step|home)\/
#   [MITM] hostname = %APPEND% openapp.fmy90.com
# Quantumult X:
#   [rewrite_local] ^https:\/\/openapp\.fmy90\.com\/(user|shop|sign|active|step|home)\/ url script-请求-header https://gist.githubusercontent.com/Sliverkiss/d744c74d780f47cd85f6980ca5014170/raw/fyhs.js
#   [mitm] hostname = openapp.fmy90.com
# Shadowrocket:
#   [Script] http-请求 https://gist.githubusercontent.com/Sliverkiss/d744c74d780f47cd85f6980ca5014170/raw/fyhs.js requires-body = false, pattern = ^https:\/\/openapp\.fmy90\.com\/(user|shop|sign|active|step|home)\/
#   [MITM] hostname = openapp.fmy90.com
...
async function checkEnv() { try { if (!userCookie?.length) throw new Error("no available accounts found"); const list=[]; for(const o of userCookie){ // 兼容旧单层结构(只有 token, 无 A/B) 视为端A
