...
------------------------------------------
@Date: 2026.08.31
@Description: 飞蚂蚁回收 双小程序签到/任务 (openapp.fmy90.com, 四端抓包模式)
new Env("飞蚂蚁回收");
...
async function main() {
    ...
    if (N === 0) {
        $.log("⚠️ 没有可执行的账号，脚本结束。请先通过抓包脚本获取 token。");
        return;
    }
...
class UserInfo {
    ...
    async turntableInfo() {
        try {
            ...
            // 奖品列表在 data.data.prizeList (源码确认)
    ...
    async draw() {
        try {
            // 转盘抽奖开关: fmy_draw, 设为 true/1/yes/on 才抽 (默认关闭)
    ...
    // B端专属: 答题红包(answer-invite) — 无"提交"接口,点击即判断对错→直接领奖开红包
    // 链路(HAR实测): user-info(GET query带activityId) -> bind(POST body activity_id) -> send-self-answer-reward -> open-red-reward
    async answerInvite() {
        const acID = 3; // activityId / activity_id (HAR确认)
        const base = "/answer-invite";
        // GET: 参数在URL query, 字段活动ID用驼峰 activityId
        const gf = (p) => this.fetch({ url: `${base}/${p}${/\?/.test(p) ? '&' : '?'}activityId=${acID}`, type: "get" });
        // POST: body 字段用下划线 activity_id (fetch自动注入公参+platformKey)
        const pf = (p, b) => this.fetch({ url: `${base}/${p}`, body: { "activity_id": acID, ...b } });
        try {
            let isAnswered = "0";
            // 1. 查答题状态 (GET, 带 activityId query)
            const info = await gf("user-info");
            if (info?.code == 200) isAnswered = String(info?.data?.answer_user?.is_answered ?? "0");

            // 2. 无论是否已答过, 始终执行任务链 (task-list -> complete-task 逐个领奖励)
            let completed = 0;
            try {
                ...
                if (Array.isArray(tasks)) {
                    for (const t of tasks) {
                        if (String(t.is_complete) === "1") continue; // 已完成跳过
                        const tid = t.id; // HAR确认: complete-task 的 task_id = 任务的 id
                        if (tid == null) continue;
                        // 外部操作类任务(加小程序/关注/加好友) complete-task 必"无效任务", 跳过
                        const sign = String(t.sign || '');
                        if (/COLLECT_MINI_PROGRAM|FOLLOW_EWX|ADD_EWX|COLLECT|FOLLOW|ADD/.test(sign)) {
                            $.log(`[${this.userName}] 答题红包任务: ${t.title||tid} 需手动(外部操作),跳过`);
                            continue;
                        }
                        const ct = await pf("complete-task", { "task_id": tid });
                        if (ct?.data?.reward_log?.id) { completed++; $.log(`[${this.userName}] 答题红包任务: ${t.title||t.typeName||tid} 完成+${(t.reward_value||10)/100}元`); }
                        else $.log(`[${this.userName}] 答题红包任务: ${t.title||t.typeName||tid} ${ct?.message||'未完成'}`);
                        await sdelay(1000, 2500);
                    }
                }
            } catch (e) { $.log(`[${this.userName}] 答题红包任务: ${e}`); }
            if (completed) $.info(`[${this.userName}] 答题红包: 完成 ${completed} 个任务`);

            // 3. 绑定 (POST)
            const bind = await pf("bind", { "answer_invite_share_key": "", "config_id": acID });
            $.log(`[${this.userName}] 答题红包: 已绑定`);

            // 4. 先答题领奖 (未答过才调 send-self-answer-reward; 已答过跳过)
            let hadAnswer = String(isAnswered) === "1";
            if (!hadAnswer) {
                const reward = await pf("send-self-answer-reward", {});
                const rid = reward?.data?.reward_log?.id;
                if (rid) { $.log(`[${this.userName}] 答题红包: 答对奖励 reward_id=${rid}`); }
                else $.log(`[${this.userName}] 答题红包: 答题${reward?.message||'无奖励'}`);
            } else {
                $.log(`[${this.userName}] 答题红包: 今日已答过`);
            }

            // 5. 再领红包 (user-reward-list 找 status:0, 逐个 open-red-reward) [对所有人, 不跳]
            let opened = 0, totalAmt = 0;
            try {
                const rl = await gf("user-reward-list?page=1&pageSize=20");
                // HAR实证: 红包数组在 data.data.data (分页结构 {total, data:[...]}), 非 data.list
                const pag = rl?.data?.data || {};
                const rew = Array.isArray(pag) ? pag : (pag.data || rl?.data?.list || rl?.data || []);
                if (Array.isArray(rew)) {
                    for (const r of rew) {
                        if (String(r.status) !== "0") continue; // 仅开未开的(status:0)
                        if (r.id == null) continue;
                        const open = await pf("open-red-reward", { "reward_id": r.id });
                        const amt = open?.data?.amount ?? r.amount; // 单位:分
                        if (amt != null) { opened++; totalAmt += Number(amt); $.log(`[${this.userName}] 答题红包: 开红包 +${(amt/100).toFixed(2)}元`); }
                        else $.log(`[${this.userName}] 答题红包: reward=${r.id} ${open?.message||'已开或失败'}`);
                        await sdelay(1000, 2500);
                    }
                }
            } catch (e) { $.log(`[${this.userName}] 开红包: ${e}`); }
            if (opened) $.info(`[${this.userName}] 答题红包: 领取 ${opened} 个红包, +${(totalAmt/100).toFixed(2)}元 ✅`);
            else $.log(`[${this.userName}] 答题红包: 无可领取红包`);
        } catch (e) {
...
// 获取Cookie (四端抓包双模式)
//  方式1(推荐, 自动触发): script-request-header 拦业务接口请求头 Authorization(token), 无需重登
//  方式2(信息全): script-response-body 拦 /auth/wx/login 响应(data.token/data.user.userId/手机号), 需重登触发
//  统一按 Referer appId 判断端A/端B
async function getCookie() {
    try {
        // 分端: 优先看请求 URL 的 platformKey(每请求必带且两端固定不同, 最可靠), 其次 Referer appId
        //  端A platformKey=F2EE2489... / 端B platformKey=9F7BCAA9...
        let endKey = 'A';
        try {
        ...
        if (typeof $response !== 'undefined' && $response && $response.body) {
            // 方式2: 拦 login 响应, 信息全(含 userId/手机号)
        ...
        if (!token) return;   // 无 token 直接放行, 不污染存储

        // 入库: 有 userId 按 userId 归并(可做同会员跳过), 无 userId 按 token 归并
        let acc, idx;
        if (userId) {
            idx = userCookie.findIndex(e => String(e.userId) === userId);
        } else {
            // 无 userId: 若该端已存在同 token 则跳过, 否则新增账号条目
            idx = userCookie.findIndex(e => e[endKey] && e[endKey].token && e[endKey].token === token);
            if (idx < 0) idx = userCookie.findIndex(e => !e[endKey] || !e[endKey].token); // 复用空端槽位
        }
        if (idx >= 0) {
            acc = userCookie[idx];
            if (typeof acc.token === 'string' && !acc.A) { acc.A = { token: acc.token }; } // 迁移旧单端
            delete acc.token;
        } else {
            acc = { userId: userId || '', userName: '', A: {}, B: {} };
            userCookie.push(acc);
        }
        if (userId && userName) acc.userName = acc.userName || userName;
        if (!acc.A) acc.A = {};
        if (!acc.B) acc.B = {};
        acc[endKey] = { token };

        $.setjson(userCookie, ckName);
        const uname = maskString(userName || (userId ? userName : token.replace(/^bearer\s/i,'')) || endKey);
...
// ===== 远端通知 (强制模块: CDN 加载, 30分钟缓存, 失败不影响业务) =====
...
async function checkEnv() { 
  try { 
    if (!userCookie?.length) { 
      $.log("❌ 未检测到任何账号数据，请先通过抓包脚本获取 token！"); 
      return; // 直接返回，不抛异常
    } 
    const list=[]; 
    for(const o of userCookie){ 
      const hasA = !!((o && o.A && o.A.token) || (o && o.token)); 
      const hasB = !!(o && o.B && o.B.token);
      if(hasA) list.push(new UserInfo(o,'A')); 
      if(hasB) list.push(new UserInfo(o,'B')); 
    } 
...
function Env(t, e) { class  { constructor(t) { this.env = t } send(t, e = "GET") { t = "string" == typeof t ? { url: t } : t; let  = this.get; return "POST" === e && ( = this.post), new Promise(((e, i) => { .call(this, t, ((t, , o) => { t ? i(t) : e() })) })) } get(t) { return this.send.call(this.env, t) } post(t) { return this.send.call(this.env, t, "POST") } } return new class { constructor(t, e) { this.logLevels = { debug: 0, info: 1, warn: 2, error: 3 }, this.logLevelPrefixs = { debug: "[DEBUG] ", info: "[INFO] ", warn: "[WARN] ", error: "[ERROR] " }, this.logLevel = "info", this.name = t, this.http = new (this), this.data = null, this.dataFile = "box.dat", this.logs = [], this.isMute = !1, this.isNeedRewrite = !1, this.logSeparator = "\n", this.encoding = "utf-8", this.startTime = (new Date).getTime(), Object.assign(this, e), this.log("", `🔔${this.name}, 开始!`) } getEnv() { return "undefined" != typeof $environment && $environment["surge-version"] ? "Surge" : "undefined" != typeof $environment && $environment["stash-version"] ? "Stash" : "undefined" != typeof module && module.exports ? "Node.js" : "undefined" != typeof $task ? "Quantumult X" : "undefined" != typeof $loon ? "Loon" : "undefined" != typeof $rocket ? "Shadowrocket" : void 0 } isNode() { return "Node.js" === this.getEnv() } isQuanX() { return "Quantumult X" === this.getEnv() } isSurge() { return "Surge" === this.getEnv() } isLoon() { return "Loon" === this.getEnv() } isShadowrocket() { return "Shadowrocket" === this.getEnv() } isStash() { return "Stash" === this.getEnv() } toObj(t, e = null) { try { return JSON.parse(t) } catch(_){ return e } } toStr(t, e = null, ...) { try { return JSON.stringify(t, ...) } catch(_){ return e } } getjson(t, e) { let  = e; if (this.getdata(t)) try {  = JSON.parse(this.getdata(t)) } catch(_){ } return  } setjson(t, e) { try { return this.setdata(JSON.stringify(t), e) } catch(_){ return !1 } } getScript(t) { return new Promise((e => { this.get({ url: t }, ((t, , i) => e(i))) })) } runScript(t, e) { return new Promise(( => { let i = this.getdata("@chavy_boxjs_userCfgs.httpapi"); i = i ? i.replace(/\n/g, "").trim() : i; let o = this.getdata("@chavy_boxjs_userCfgs.httpapi_timeout"); o = o ? 1 * o : 20, o = e && e.timeout ? e.timeout : o; const [r, a] = i.split("@"), n = { url: `http://${a}/v1/scripting/evaluate`, body: { script_text: t, mock_type: "cron", timeout: o }, headers: { "X-Key": r, Accept: "*/*" }, timeout: o }; this.post(n, ((t, e, i) => (i))) })).catch((t => this.logErr(t))) } loaddata() { if (!this.isNode()) return {}; { this.fs = this.fs ? this.fs : require("fs"), this.path = this.path ? this.path : require("path"); const t = this.path.resolve(this.dataFile), e = this.path.resolve(process.cwd(), this.dataFile),  = this.fs.existsSync(t), i = ! && this.fs.existsSync(e); if (! && !i) return {}; { const i =  ? t : e; try { return JSON.parse(this.fs.readFileSync(i)) } catch (t) { return {} } } } } writedata() { if (this.isNode()) { this.fs = this.fs ? this.fs : require("fs"), thi.path = this.path ? thi.path : require("path"); const t = this.path.resolve(this.dataFile), e = this.path.resolve(process.cwd(), this.dataFile),  = this.fs.existsSync(t), i = ! && this.fs.existsSync(e), o = JSON.stringify(this.data);  ? this.fs.writeFileSync(t, o) : i ? this.fs.writeFileSync(e, o) : this.fs.writeFileSync(t, o) } } lodash_get(t, e, ) { const i = e.replace(/\[(\d+)\]/g, ".$1").split("."); let o = t; for (const t of i) if (o = Object(o)[t], void 0 === o) return ; return o } lodash_set(t, e, ) { return Object(t) !== t || (Array.isArray(e) || (e = e.toString().match(/[^.[\]]+/g) || []), e.slice(0, -1).reduce(((t, , i) => Object(t[]) === t[] ? t[] : t[] = Math.abs(e[i + 1]) >> 0 == +e[i + 1] ? [] : {}), t)[e[e.length - 1]] = ), t } getdata(t) { let e = this.getval(t); if (/^@/.test(t)) { const [, , i] = /^@(.*?)\.(.*?)$/.exec(t), o =  ? this.getval() : ""; if (o) try { const t = JSON.parse(o); e = t ? this.lodash_get(t, i, "") : e } catch (t) { e = "" } } return e } setdata(t, e) { let  = !1; if (/^@/.test(e)) { const [, i, o] = /^@(.*?)\.(.*?)$/.exec(e), r = this.getval(i), a = i ? "null" === r ? null : r || "{}" : "{}"; try { const e = JSON.parse(a); this.lodash_set(e, o, t),  = this.setval(JSON.stringify(e), i) } catch (e) { const r = {}; this.lodash_set(r, o, t),  = this.setval(JSON.stringify(r), i) } } else  = this.setval(t, e); return  } getval(t) { switch (this.getEnv()) { case "Surge": case "Loon": case "Stash": case "Shadowrocket": return $persistentStore.read(t); case "Quantumult X": return $prefs.valueForKey(t); case "Node.js": return this.data = this.loaddata(), this.data[t]; default: return this.data && this.data[t] || null } } setval(t, e) { switch (this.getEnv()) { case "Surge": case "Loon": case "Stash": case "Shadowrocket": return $persistentStore.write(t, e); case "Quantumult X": return $prefs.setValueForKey(t, e); case "Node.js": return this.data = this.loaddata(), this.data[e] = t, this.writedata(), !0; default: return this.data && this.data[e] || null } } initGotEnv(t) { this.got = this.got ? this.got : require("got"), this.cktough = this.cktough ? this.cktough : require("tough-cookie"), this.ckjar = this.ckjar ? this.ckjar : new this.cktough.CookieJar, t && (t.headers = t.headers ? t.headers : {}, t && (t.headers = t.headers ? t.headers : {}, void 0 === t.headers.cookie && void 0 === t.headers.Cookie && void 0 === t.cookieJar && (t.cookieJar = this.ckjar))) } get(t, e = (() => { })) { switch (t.headers && (delete t.headers["Content-Type"], delete t.headers["Content-Length"], delete t.headers["content-type"], delete t.headers["content-length"]), t.params && (t.url += "?" + this.queryStr(t.params)), void 0 === t.followRedirect || t.followRedirect || ((this.isSurge() || this.isLoon()) && (t["auto-redirect"] = !1), this.isQuanX() && (t.opts ? t.opts.redirection = !1 : t.opts = { redirection: !1 })), this.getEnv()) { case "Surge": case "Loon": case "Stash": case "Shadowrocket": default: this.isSurge() && this.isNeedRewrite && (t.headers = t.headers || {}, Object.assign(t.headers, { "X-Surge-Skip-Scripting": !1 })), $httpClient.get(t, ((t, , i) => { !t &&  && (.body = i, .statusCode = .status ? .status : .statusCode, .status = .statusCode), e(t, , i) })); break; case "Quantumult X": this.isNeedRewrite && (t.opts = t.opts || {}, Object.assign(t.opts, { hints: !1 })), $task.fetch(t).then((t => { const { statusCode: , statusCode: i, headers: o, body: r, bodyBytes: a } = t; e(null, { status: , statusCode: i, headers: o, body: r, bodyBytes: a }, r, a) }), (t => e(t && t.error || "UndefinedError"))); break; case "Node.js": let  = require("iconv-lite"); this.initGotEnv(t), this.got(t).on("redirect", ((t, e) => { try { if (t.headers["set-cookie"]) { const  = t.headers["set-cookie"].map(this.cktough.Cookie.parse).toString();  && this.ckjar.setCookieSync(, null), e.cookieJar = this.ckjar } } catch (t) { this.logErr(t) } })).then((t => { const { statusCode: i, statusCode: o, headers: r, rawBody: a } = t, n = .decode(a, this.encoding); e(null, { status: i, statusCode: o, headers: r, rawBody: a, body: n }, n) }), (t => { const { message: i, response: o } = t; e(i, o, o && .decode(o.rawBody, this.encoding)) })); break } } post(t, e = (() => { })) { const  = t.method ? t.method.toLocaleLowerCase() : "post"; switch (t.body && t.headers && !t.headers["Content-Type"] && !t.headers["content-type"] && (t.headers["content-type"] = "application/x-www-form-urlencoded"), t.headers && (delete t.headers["Content-Length"], delete t.headers["content-length"]), void 0 === t.followRedirect || t.followRedirect || ((this.isSurge() || this.isLoon()) && (t["auto-redirect"] = !1), this.isQuanX() && (t.opts ? t.opts.redirection = !1 : t.opts = { redirection: !1 })), this.getEnv()) { case "Surge": case "Loon": case "Stash": case "Shadowrocket": default: this.isSurge() && this.isNeedRewrite && (t.headers = t.headers || {}, Object.assign(t.headers, { "X-Surge-Skip-Scripting": !1 })), $httpClient[](t, ((t, , i) => { !t &&  && (.body = i, .statusCode = .status ? .status : .statusCode, .status = .statusCode), e(t, , i) })); break; case "Quantumult X": t.method = , this.isNeedRewrite && (t.opts = t.opts || {}, Object.assign(t.opts, { hints: !1 })), $task.fetch(t).then((t => { const { statusCode: , statusCode: i, headers: o, body: r, bodyBytes: a } = t; e(null, { status: , statusCode: i, headers: o, body: r, bodyBytes: a }, r, a) }), (t => e(t && t.error || "UndefinedError"))); break; case "Node.js": let i = require("iconv-lite"); this.initGotEnv(t); const { url: o, ...r } = t; this.got[](o, r).then((t => { const { statusCode: , statusCode: o, headers: r, rawBody: a } = t, n = i.decode(a, this.encoding); e(null, { status: , statusCode: o, headers: r, rawBody: a, body: n }, n) }), (t => { const { message: , response: o } = t; e(, o, o && i.decode(o.rawBody, this.encoding)) })); break } } time(t, e = null) { const  = e ? new Date(e) : new Date; let i = { "M+": .getMonth() + 1, "d+": .getDate(), "H+": .getHours(), "m+": .getMinutes(), "s+": .getSeconds(), "q+": Math.floor((.getMonth() + 3) / 3), S: .getMilliseconds() }; /(y+)/.test(t) && (t = t.replace(RegExp.$1, (.getFullYear() + "").substr(4 - RegExp.$1.length))); for (let e in i) new RegExp("(" + e + ")").test(t) && (t = t.replace(RegExp.$1, 1 == RegExp.$1.length ? i[e] : ("00" + i[e]).substr(("" + i[e]).length))); return t } queryStr(t) { let e = ""; for (const  in t) { let i = t[]; null != i && "" !== i && ("object" == typeof i && (i = JSON.stringify(i)), e += `${}=${i}&`) } return e = e.substring(0, e.length - 1), e } msg(e = t,  = "", i = "", o = {}) { const r = t => { const { $open: e, $copy: , $media: i, $mediaMime: o } = t; switch (typeof t) { case void 0: return t; case "string": switch (this.getEnv()) { case "Surge": case "Stash": default: return { url: t }; case "Loon": case "Shadowrocket": return t; case "Quantumult X": return { "open-url": t }; case "Node.js": return }case "object": switch (this.getEnv()) { case "Surge": case "Stash": case "Shadowrocket": default: { const r = {}; let a = t.openUrl || t.url || t["open-url"] || e; a && Object.assign(r, { action: "open-url", url: a }); let n = t["update-pasteboard"] || t.updatePasteboard || ; if (n && Object.assign(r, { action: "clipboard", text: n }), i) { let t, e, ; if (i.startsWith("http")) t = i; else if (i.startsWith("data:")) { const [t] = i.split(";"), [, o] = i.split(","); e = o,  = t.replace("data:", "") } else { e = i,  = (t => { const e = { JVBERi0: "application/pdf", R0lGODdh: "image/gif", R0lGODlh: "image/gif", iVBORw0KGgo: "image/png", "/9j/": "image/jpg" }; for (var  in e) if (0 === t.indexOf()) return e[]; return null })(i) } Object.assign(r, { "media-url": t, "media-base64": e, "media-base64-mime": o ??  }) } return Object.assign(r, { "auto-dismiss": t["auto-dismiss"], ound: t.sound }), r } case "Loon": { const  = {}; let o = t.openUrl || t.url || t["open-url"] || e; o && Object.assign(, { openUrl: o }); let r = t.mediaUrl || t["media-url"]; return i?.startsWith("http") && (r = i), r && Object.assign(, { mediaUrl: r }), console.log(JSON.stringify()),  } case "Quantumult X": { const o = {}; let r = t["open-url"] || t.url || t.openUrl || e; r && Object.assign(o, { "open-url": r }); let a = t["media-url"] || t.mediaUrl; i?.startsWith("http") && (a = i), a && Object.assign(o, { "media-url": a }); let n = t["update-pasteboard"] || t.updatePasteboard || ; return n && Object.assign(o, { "update-pasteboard": n }), console.log(JSON.stringify(o)), o } case "Node.js": return }default: return } }; if (!this.isMute) switch (this.getEnv()) { case "Surge": case "Loon": case "Stash": case "Shadowrocket": default: $notification.post(e, , i, r(o)); break; case "Quantumult X": $notify(e, , i, r(o)); break; case "Node.js": break }if (!this.isMuteLog) { let t = ["", "==============📣系统通知📣=============="]; t.push(e),  && t.push(), i && t.push(i), console.log(t.join("\n")), this.logs = this.logs.concat(t) } } debug(...t) { this.logLevels[this.logLevel] <= this.logLevels.debug && (t.length > 0 && (this.logs = [...this.logs, ...t]), console.log(`${this.logLevelPrefixs.debug}${t.map((t => t ?? String(t))).join(this.logSeparator)}`)) } info(...t) { this.logLevels[this.logLevel] <= this.logLevels.info && (t.length > 0 && (this.logs = [...this.logs, ...t]), console.log(`${this.logLevelPrefixs.info}${t.map((t => t ?? String(t))).join(this.logSeparator)}`)) } warn(...t) { this.logLevels[this.logLevel] <= this.logLevels.warn && (t.length > 0 && (this.logs = [...this.logs, ...t]), console.log(`${this.logLevelPrefixs.warn}${t.map((t => t ?? String(t))).join(this.logSeparator)}`)) } error(...t) { this.logLevels[this.logLevel] <= this.logLevels.error && (t.length > 0 && (this.logs = [...this.logs, ...t]), console.log(`${this.logLevelPrefixs.error}${t.map((t => t ?? String(t))).join(this.logSeparator)}`)) } log(...t) { t.length > 0 && (this.logs = [...this.logs, ...t]), console.log(t.map((t => t ?? String(t))).join(this.logSeparator)) } logErr(t, e) { switch (this.getEnv()) { case "Surge": case "Loon": case "Stash": case "Shadowrocket": case "Quantumult X": default: this.log("", `❗️${this.name}, 错误!`, e, t); break; case "Node.js": this.log("", `❗️${this.name}, 错误!`, e, void 0 !== t.message ? t.message : t, t.stack); break } } wait(t) { return new Promise((e => setTimeout(e, t))) } done(t = {}) { const e = ((new Date).getTime() - this.startTime) / 1e3; switch (this.log("", `🔔${this.name}, 结束! 🕛 ${e} 秒`), this.log(), this.getEnv()) { case "Surge": case "Loon": case "Stash": case "Shadowrocket": case "Quantumult X": default: $done(t); break; case "Node.js": process.exit(1) } } }(t, e) }
